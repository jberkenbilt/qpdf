...
{
ap.addHelpTopic("usage", "basic invocation", R"(Usage: qpdf infile [options] [outfile]
   OR  qpdf help-option
...
ap.addHelpTopic("exit-status", "exit status", R"(Meaning of exit codes:
...
ap.addOptionHelp("--completion-bash", "completion", "output command for bash completion", R"()");
ap.addOptionHelp("--completion-zsh", "completion", "output command for zsh completion", R"()");
...
ap.addOptionHelp("--help", "help", "provide help", R"()");
ap.addOptionHelp("--version", "help", "show qpdf version", R"()");
ap.addOptionHelp("--copyright", "help", "show copyright information", R"()");
...
{
ap.addOptionHelp("--deterministic-id", "general", "generate ID deterministically", R"(Causes generation a secure, random document ID only using
...
ap.addHelpTopic("advanced-control", "advanced control", R"(Advanced control options control qpdf's behavior in ways that would
...
ap.addOptionHelp("--ignore-xref-streams", "advanced-control", "ignore xref streams", R"()");
ap.addHelpTopic("transformation", "structural PDF changes", R"(Transformation options tell qpdf to create output files with
certain properties. These options change the structure without
changing the content.
)");
ap.addOptionHelp("--linearize", "transformation", "linearize (web-optimize) output", R"()");
...
ap.addOptionHelp("--decrypt", "transformation", "remove encryption from input file", R"(Normally qpdf preserves whatever encryption was present on the
input file. This causes qpdf to write an unencrypted output even
if the input was encrypted.
)");
ap.addOptionHelp("--copy-encryption", "transformation", "copy encryption details", R"(--copy-encryption=file
...
ap.addOptionHelp("--qdf", "transformation", "enable viewing in a text editor", R"(Create a PDF file suitable for viewing in a text editor and even
...
ap.addOptionHelp("--stream-data", "transformation", "control stream compression (legacy)", R"(--stream-data=option
...
ap.addOptionHelp("--preserve-unreferenced-resources", "transformation", "same as --remove-unreferenced-resources=no", R"()");
...
ap.addOptionHelp("--externalize-inline-images", "transformation", "convert inline images to regular images", R"()");
ap.addOptionHelp("--ii-min-bytes", "transformation", "set minimize for --externalize-inline-images", R"(--ii-min-bytes=bytes
...
ap.addHelpTopic("modification", "changes parts of the PDF", R"(Modification options make systematic changes to certain parts of
...
ap.addOptionHelp("--flatten-rotation", "modification", "remove rotate from page dictionary", R"(Rotate a page using content commands instead of page-level
...
ap.addOptionHelp("--list-attachments", "attachments", "list embedded files", R"(Show the key and stream number for each embedded files. Combine
...
{
ap.addHelpTopic("add-attachment", "adding attachments", R"(The options listed below appear between --add-attachment and its
terminating "--".
...
ap.addOptionHelp("--filename", "add-attachment", "name of attached file", R"(--filename=name
...
ap.addOptionHelp("--creationdate", "add-attachment", "short text", R"(--creationdate=date
...
ap.addOptionHelp("--moddate", "add-attachment", "short text", R"(--moddate=date
...
ap.addOptionHelp("--description", "add-attachment", "description for an attachment", R"(--description="text"
...
ap.addHelpTopic("copy-attachments", "copying attachments", R"(The options listed below appear between --copy-attachments-from and
its terminating "--".
...
ap.addOptionHelp("--is-encrypted", "copy-attachments", "silent test for encrypted file", R"(Silently exit with status 0 if the file is encrypted or status 2
if the file is not encrypted. This can be used with
password-protected files even if you don't know the password.
)");
ap.addOptionHelp("--requires-password", "copy-attachments", "silent test for password", R"(long text
)");
...
ap.addOptionHelp("--show-encryption", "copy-attachments", "information about encrypted files", R"(Shows document encryption parameters. Also shows the document's
...
ap.addOptionHelp("--show-linearization", "copy-attachments", "show linearization hint tables", R"(Check and displays all data in the linearization hint tables.
)");
ap.addOptionHelp("--show-xref", "copy-attachments", "show cross reference table", R"(Show the contents of the cross-reference table in a
...
ap.addOptionHelp("--show-npages", "copy-attachments", "show number of pages", R"(Prints the number of pages in the input file on a line by
...
ap.addOptionHelp("--json", "json", "show file in json format", R"(long text
)");
ap.addOptionHelp("--json-help", "json", "show format of json output", R"(Describe the format of the JSON output.
)");
ap.addOptionHelp("--json-key", "json", "restrict json output", R"(--json-key=key
...
ap.addOptionHelp("--static-id", "testing", "use a static document ID", R"(Causes generation of a fixed value for the document ID. This is
