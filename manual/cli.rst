...
.. help-topic usage: basic invocation
   ...
   - @filename is an argument file; each line is treated as
...
This converts PDF file :samp:`{infile}` to PDF file :samp:`{outfile}`.
When run with no arguments, the output file is functionally identical
to the input file but may have been structurally reorganized, and
orphaned objects have been removed from the file. Many transformations
are available as controlled by the options below.

As discussed below, you may specify :qpdf:ref:`--empty` in place of
:samp:`{infile}`, and you may specify :qpdf:ref:`--replace-input` in
place of :samp:`{outfile}`.
...
command line. Note that the :samp:`@filename` can't appear in the
middle of an argument, so constructs such as :samp:`--arg=@option`
will not work. You would have to include the argument and its options
together in the arguments file. See also :qpdf:ref:`--password-file`.

:samp:`{outfile}` does not have to be seekable, even when generating
linearized files. Specifying ":samp:`-`" as :samp:`{outfile}` means to
write to standard output. The input file *does* have to be seekable.
You can't read from standard input or a pipe.
...
.. qpdf:option:: --empty
   ...
   normal use case for using :qpdf:ref:`--empty` would be if you were
   going to add pages from another source, as discussed in
   :ref:`page-selection`.
...
.. help-topic exit-status: exit status

   Meaning of exit codes:

   0: no errors or warnings
   1: not used by qpdf but may be used by the shell if unable to
      invoke qpdf
   2: errors detected
...
.. qpdf:option:: --warning-exit-0
   ...
   instead of ``3``. When combined with :qpdf:ref:`--no-warn`, the
   effect is for :command:`qpdf` to completely ignore warnings.
...
is built from source or invoked with a wrapper script. You can specify
a full path to the qpdf you want to use for completion in the
...
.. qpdf:option:: --completion-bash
   ...
   .. help: output command for bash completion
...
.. qpdf:option:: --completion-zsh
   ...
   .. help: output command for zsh completion
...
.. qpdf:option:: --version
   ...
   Display the current version of qpdf. The version number displayed
   is the one that is compiled into the qpdf library. If you don't see
   the version number you expect, you may have more than one version
   of :command:`qpdf` installed and may not have your library path set
   up correctly.
...
:qpdf:ref:`--password`. By default, qpdf will preserve any encryption
data associated with a file. If :qpdf:ref:`--decrypt` is specified,
qpdf will attempt to remove any encryption information. If the file is
being encrypted (see :ref:`encryption-options`), specified options
take precedence.
...
.. qpdf:option:: --password=password
   ...
   use :qpdf:ref:`--password-file`, added in qpdf 10.2. You can also
   use :samp:`@filename` or :samp:`@-` (see :ref:`invocation`) to put
   the password in a file or pass it via standard input, but you would
   do so by specifying the entire :samp:`--password={password}` option
   in the file. Syntax such as :samp:`--password=@filename` won't work
   since :samp:`@filename` is not recognized in the middle of an
   argument.
...
.. qpdf:option:: --password-file=filename
   ...
   ``-`` to read the password from standard input. Note that, in this
   case, the password is echoed and there is no prompt, so use with
   caution.
...
.. qpdf:option:: --progress
   ...
   Indicate progress while writing output files. Note that progress
   indication does not start until writing starts, so if complicated
...
.. qpdf:option:: --deterministic-id
   ...
   .. help: generate ID deterministically
      ...
      Causes generation a secure, random document ID only using
      static information, such as the page contents. Do not use the
      file's name or attributes or the current time.

   Causes generation of a secure, random document ID using
   deterministic values. This prevents use of timestamp and output
   file name information in the ID generation. Instead, at some slight
...
.. qpdf:option:: --keep-files-open-threshold=count
   ...
   .. help: set threshold for --keep-files-open
      ...
      By default, when more than 200 files are in use, qpdf opens
      files only when needed and closes them afterwards. This option
...
.. help-topic advanced-control: advanced control

   Advanced control options control qpdf's behavior in ways that would
   normally never be needed by a user but may be useful to developers
   or people investigating problems with specific files.

Advanced control options control qpdf's behavior in ways that would
normally never be needed by a user but may be useful to developers or
people investigating problems with specific files.
...
.. qpdf:option:: --password-mode={mode}
   ...
   - :samp:`bytes`: Interpret the password as a literal byte string.
     ...
     string of bytes as provided. This option will allow you to
     encrypt PDF files with passwords that will not be usable by other
     readers.
...
.. qpdf:option:: --ignore-xref-streams
   ...
   .. help: ignore xref streams

   Tells qpdf to ignore any cross-reference streams, falling back to
   any embedded cross-reference tables or forcing document recovery.
   Ordinarily, qpdf reads cross-reference streams when they are
   present in a PDF file. If this option is specified, specified, qpdf
   will ignore any cross-reference streams for hybrid PDF files. The
   purpose of hybrid files is to make some content available to
...
.. help-topic transformation: structural PDF changes

   Transformation options tell qpdf to create output files with
   certain properties. These options change the structure without
   changing the content.

Transformation options tell qpdf to create output files with certain
properties. These options change the structure without changing the
content. These options can be used for a variety of reasons, such as
creating linearized (web-optimized) files, restructuring files for
older viewers, rewriting files for human inspection, preparing files
for print, among others. See also :ref:`modification-options`.
...
.. qpdf:option:: --linearize
   ...
   of the file. While :command:`qpdf` creates linearized files that
   most major browsers recognize as such, it does not support all
   possible linearization options nor does it fully populate all the
...
.. qpdf:option:: --encrypt user owner key-length [ options ] --
   ...
   .. help: start encryption options
      ...
      Usage: --encrypt user-password owner-password key-length [ options ] --
      Run qpdf --help=encryption for details.

   This flag starts encryption options, used to create encrypted
   files.

   Usage: :samp:`--encrypt {user-password} {owner-password}
   {key-length} [ {options} ] --`
...
.. qpdf:option:: --decrypt
   ...
   .. help: remove encryption from input file
      ...
      Normally qpdf preserves whatever encryption was present on the
      input file. This causes qpdf to write an unencrypted output even
      if the input was encrypted.

   Normally qpdf preserves whatever encryption was present on the
   input file. This causes qpdf to write an unencrypted output even if
   the input was encrypted. See also :qpdf:ref:`--copy-encryption`.

.. qpdf:option:: --copy-encryption=file

   .. help: copy encryption details

      Copy encryption details from the specified file instead of
      preserving the input file's encryption.
      Use --encryption-file-password to specify the encryption file's
      password.

   Ordinarily, when the input file is encrypted, :command:`qpdf` uses
   the same encryption settings for the output file, unless
   :qpdf:ref:`--decrypt` is specified, in which case encryption is
   removed. This option causes :command:`qpdf` to copy the encryption
   parameters from the specified file instead. This includes the user
   password, the owner password, and all the document restrictions.
   This works even if only one of the user password or owner password
   are known. If the encryption file requires a password, use the
   ...
   without having to manual specify all the individual settings.
...
.. qpdf:option:: --qdf
   ...
   .. help: enable viewing in a text editor

      Create a PDF file suitable for viewing in a text editor and even
      editing. All streams that can be uncompressed are uncompressed,
   ...
   Create a PDF file suitable for viewing in a text editor and even
   editing. All streams that can be uncompressed are uncompressed, and
   content streams are normalized, among other changes. The companion
   tool :command:`fix-qdf` can be used to repair hand-edited QDF
   files. QDF is a feature specific to the qpdf tool. For additional
   information about QDF mode, see :ref:`qdf`. Note that
...
.. qpdf:option:: --compress-streams=[yn]
   ...
   compress any stream with no other filters applied to it with the
   ``/FlateDecode`` filter when it writes to the output file. To
   suppress this behavior and preserve uncompressed streams as
   uncompressed, use :samp:`--compress-streams=n`. In QDF mode (see
   :ref:`qdf` and :qpdf:ref:`--qdf`), the default is to leave streams
   uncompressed.
...
.. qpdf:option:: --decode-level=option
   ...
   - :samp:`generalized`: decode streams filtered with supported
     ...
     designed for image data. Note that, by default, streams already
     compressed with ``/FlateDecode`` are not uncompressed and
     recompressed unless you also specify
   ...
   uncompress and recompress streams. Assuming proper filter
   implements, this is safe and generally results in smaller files.
   For example, LZW will be replaced automatically with flate, and
   ASCII filters will be removed. This can be disabled with
   ``--decode-level=none``.

.. qpdf:option:: --stream-data=option

   .. help: control stream compression (legacy)

      This is an older argument for controlling stream data. It will
      continue to work but has been superseded by --compress-streams
      and --decode-level.
...
.. qpdf:option:: --recompress-flate
   ...
   .. help: uncompress and recompress flate
      ...
      recompress. This can be useful when combined with
      --compression-level.

   The default generalized compression scheme used by PDF is
   ``/FlateDecode``, which is the same as used by :command:`zip` and
   :command:`gzip`. Usually qpdf just leaves these alone. This option
   tells :command:`qpdf` to uncompress and recompress. This can be
   useful when combined with :qpdf:ref:`--compression-level`. Using
   this option may make :command:`qpdf` much slower when writing
   output files.
...
.. qpdf:option:: --compression-level=level
   ...
   that :command:`qpdf` doesn't uncompress and recompress streams by
   default. To have this option apply to already compressed streams,
   you should also specify :qpdf:ref:`--recompress-flate`. If your
   goal is to shrink the size of PDF files, you should also use
...
.. qpdf:option:: --normalize-content=[yn]
   ...
   .. help: fix newlines in content streams
      ...
      Normalize newlines to UNIX-style newline characters in PDF
      content streams, which is useful for viewing them in a
      programmer's text editor across multiple platforms. This is
      also turned on by --qdf.

   Enables or disables normalization of newline characters in PDF
   content streams to UNIX-style newline, which is useful for viewing
   files in a programmer-friendly text edit across multiple platforms.
   Content normalization is off by default, but is automatically
   enabled by :qpdf:ref:`--qdf` (see also :ref:`qdf`). It is not
   recommended to use this option for production use.
   ...
   token such as a string, name, or inline image. Note that files that
   do this are invalid since the PDF specification states that content
   streams are not to be split in the middle of a token. If you want
   to inspect the original content streams in an uncompressed format,
   you can always run with :samp:`--qdf --normalize-content=n` to
   create a QDF file without content normalization, or alternatively
   :samp:`--stream-data=uncompress` for a regular non-QDF mode file
   with uncompressed streams. These will both uncompress all the
   streams but will not attempt to normalize content. Please note that
   if you are using content normalization or QDF mode for the purpose
   of manually inspecting files, you don't have to care about this.
...
.. qpdf:option:: --object-streams=mode
   ...
   .. help: control use of object streams
      ...
      - generate: create object streams and include as many objects as
        possible in them
   ...
   - :samp:`generate`: create object streams and include as many
     objects as possible in them
   ...
   into the PDF specification at version 1.5. Some older PDF viewers
   may not support files with object streams. qpdf can be used to
   transform files with object streams to files without object streams
   or vice versa.
   ...
   streams. This will usually result in more compact PDF files, though
   they may not be readable by older viewers. In this mode, qpdf will
   also make sure the PDF version number in the header is at least
   1.5.
...
.. qpdf:option:: --preserve-unreferenced-resources
   ...
   .. help: same as --remove-unreferenced-resources=no
...
.. qpdf:option:: --newline-before-endstream
   ...
   Tells qpdf to insert a newline before the ``endstream`` keyword,
...
.. qpdf:option:: --coalesce-contents
   ...
   reasons. Common reasons include making it easier to modify page
   contents and splitting very large content streams so PDF viewers
   may be able to use less memory.

.. qpdf:option:: --externalize-inline-images

   .. help: convert inline images to regular images
...
.. qpdf:option:: --ii-min-bytes=bytes

   .. help: set minimize for --externalize-inline-images
...
.. qpdf:option:: --min-version=version
   ...
   .. help: set minimum PDF version
      ...
      Forces the PDF version of the output to be at least the
      specified version.
...
.. qpdf:option:: --force-version=version
   ...
   .. help: set output PDF version
      ...
      Force the output PDF file's PDF version head to be the specified
...
.. help-topic modification: changes parts of the PDF
...
.. qpdf:option:: --pages file [ --password=password ] [ page-range ] [ ... ] --
   ...
   .. help: begin page selection
      ...
      Usage: --pages input-file [ --password=password ] [ page-range ] [ ... ] --
      Run qpdf --help=page-selection for details.

   This flag starts page selection options, used to select pages from
   one or more input files to perform operations such as splitting,
   merging, and collating files.

   Usage: :samp:`--pages {input-file} [ --password={password} ] [
   {page-range} ] [ ... ] --`
...
.. qpdf:option:: --collate=n
   ...
   .. help: collate with --pages
      ...
      This option causes qpdf to collate rather than concatenate pages
      specified with --pages. With a numeric argument, collate in
      groups of n. The default is 1. Run qpdf --help=page-selection
...
.. qpdf:option:: --split-pages=[n]
   ...
   option creates an empty PDF and copies a single page from the
   output into it. If you require the global data, you will have to
   run :command:`qpdf` with the :qpdf:ref:`--pages` option once for each
   file. Using :qpdf:ref:`--split-pages` is much faster if you don't
   require the global data. A future version of qpdf may support
   preservation of some document-level information.
...
.. qpdf:option:: --flatten-rotation
   ...
   .. help: remove rotate from page dictionary
...
.. qpdf:option:: --flatten-annotations=option
   ...
   .. help: push annotations into content
      ...
      Push page annotations into the content streams. This may
   ...
   used to render filled-in form fields may become out of date from
   the fields' values if the form is filled in by a program that
   doesn't know how to update the appearances. If qpdf detects this
   case, its default behavior is not to flatten those annotations
   because doing so would cause the value of the form field to be
   lost. This gives you a chance to go back and resave the form with a
   program that knows how to generate appearances. QPDF itself can
   generate appearances with some limitations. See the
...
.. qpdf:option:: --rotate=[+|-]angle[:page-range]
   ...
   .. help: rotate pages
      ...
      or 270. Run qpdf --help=page-ranges for help with page ranges.
   ...
   inspecting the content of a PDF file, it is impossible to know
   whether a page that appears to be rotate is rotated "naturally" or
   has been rotated by specifying rotation. For example, if a page
   appears to contain a portrait-mode image rotated by 90 degrees so
   that the top of the image is on the right edge of the page, there
   is no way to tell by visual inspection whether the literal top of
   the image is the top of the page or whether the literal top of the
   image is the right edge and the page is already rotated in the PDF.
   Specifying a rotation angle of ``-90`` will produce the desired
   results in either case. Use of absolute rotation angles should be
   ...
   - ``qpdf in.pdf out.pdf --rotate=+90:2,4,6 --rotate=+180:7-8``:
     rotate pages 2, 4, and 6 90 degrees clockwise from their original
     rotation
...
.. qpdf:option:: --generate-appearances
   ...
   option. Here is a summary of the limitations, most of these are not
   a problem with well-behaved PDF files:
...
.. qpdf:option:: --keep-inline-images
   ...
   This option has no affect when :qpdf:ref:`--optimize-images` is not
   specified.
...
.. help-topic encryption: create encrypted files
   ...
   strings. key-length may be 40, 128, or 256.
   ...
   insecure encrypted files. The --allow-weak-crypto flag is specified
   outside of --encrypt ... -- (e.g. before --encrypt or after --).
   ...
   Options for 128-bit or 256-bit:
     --accessibility=[yn]     allow accessibility (usually ignored)
...
a more in-depth technical discussion of PDF encryption, see
:ref:`pdf-encryption`.

There other flags that relate to encrypted files. See also
...
may be empty strings. key-length may be ``40``, ``128``, or ``256``.
...
viewers. You must use the :qpdf:ref:`--allow-weak-crypto` to create
insecure encrypted files. The :qpdf:ref:`--allow-weak-crypto` flag is
specified outside of ``--encrypt ... --`` (before ``--encrypt`` or
after ``--``).

If :samp:`{key-length}` is 256, the minimum PDF version is 1.7 with
extension level 8, and the AES-based encryption format used is the
PDF 2.0 encryption method supported by Acrobat X. Using 128-bit
encryption forces the PDF version to be at least 1.4, or if AES is
used, 1.6. Using 40-bit encryption forces the PDF version to be at
least 1.3.
...
options you specify. Programs such as :command:`qpdf` can also be used
to bypass or remove restrictions if either the user password or owner
password is known. If you specify certain combinations of restrictions
and find a reader that doesn't seem to honor them as you expect, it is
most likely not a bug in :command:`qpdf`.
...
.. qpdf:option:: --modify=modify-opt
   ...
   .. help: restrict document modification
      ...
      enabling and disabling levels of documentation in a manner
      similar to how some PDF creation tools do it. modify-opt values
      map to other combinations of options as follows:
   ...
   allow enabling and disabling levels of documentation in a manner
   similar to how some PDF creation tools do it. :samp:`{modify-opt}`
   values map to other combinations of options as follows:

   - :samp:`all`: allow full document modification (the default)

   - :samp:`annotate`: same as :samp:`--modify-other=n`

   - :samp:`form`: same as :samp:`--modify-other=n --annotate=n`

   - :samp:`assembly`: same as :samp:`--modify-other=n --annotate=n --form=n`

   - :samp:`none`: allow no modifications; same as
     :samp:`--modify-other=n --annotate=n --form=n --assemble=n`
...
.. qpdf:option:: --print=print-opt
   ...
   .. help: restrict printing
      ...
      print-opt may be y or n and enable or disable all printing. For
      128-bit and 256-bit encryption, print-opt may have the following
      values:
...
.. qpdf:option:: --allow-insecure
   ...
   where the user password is non-empty and the owner password is
   empty when 256-bit encryption is selected. Files created in this
   way are insecure since they can be opened without a password
   without restrictions being enforced. Users would ordinarily never
   want to create such files. If you are using qpdf to intentionally
   created strange files for testing (a definite valid use of qpdf!),
   this option allows you to create such insecure files. This option
   is only available with 256-bit encryption.
...
Notes:
  ...
  - Document-level information, such as outlines, tags, etc., is taken
    from the primary input file (in the above example, :samp:`in.pdf`)
    and is preserved in out.pdf. You can use :qpdf:ref:`--empty` in
    place of an input file to start from an empty file and just copy
    pages equally from all files.
...
- You can specify a numeric argument to :qpdf:ref:`--collate`. With
  ...
  if you ran :command:`qpdf --collate=2 --empty --pages a.pdf 1-5 b.pdf
  6-4 c.pdf r1 -- out.pdf`, you would get the following pages in this
  order:
...
- Take pages 1 through 5 from :file:`file1.pdf` and pages 11 through
  15 from :file:`file2.pdf` in reverse, taking document-level metadata
  from :file:`file2.pdf`.
...
- Here's a more contrived example. If, for some reason, you wanted to
  ...
  an output file, and if you wanted to drop document-level metadata
  but preserve encryption, you could run
...
the same page from the same file directly more than once, and the
...
combined with other like merging and will apply to the final output.
The ``--overlay`` and ``--underlay`` options work the same way, except
underlay pages are drawn underneath the page to which they are
applied, possibly obscured by the original page, and overlay files are
drawn on top of the page to which they are applied, possibly obscuring
the page. You can combine overlay and underlay.
...
specified, all pages will be used. This can be explicitly specified to
be empty by omitting :samp:`{page-range}` if :qpdf:ref:`--repeat` is
used.
...
up. If you want to repeat a range of pages starting at the beginning,
you can explicitly use ``--from=``.
...
attachments) and to copy attachments from other files. Information
about the individual options appears below.
...
.. qpdf:option:: --list-attachments
   ...
   .. help: list embedded files
      ...
      Show the key and stream number for each embedded files. Combine
...
.. qpdf:option:: --add-attachment file options --
   ...
   .. help: start add attachment options
      ...
      Usage: --add-attachment {file} {options} --
   ...
   This flag starts add attachment options, used to add attachments to
   a file.

   Usage: :samp:`--add-attachment {file} {options} --`
...
.. qpdf:option:: --remove-attachment=key
   ...
   file specification. That means that any potential internal links to
   the attachment will be broken. This option may be specified
   multiple times. Run with :qpdf:ref:`--verbose` to see status of the
   removal. Use :qpdf:ref:`--list-attachments` to find the attachment
   key. This option may be repeated to remove multiple attachments.

.. qpdf:option:: --copy-attachments-from file options --

   .. help: start copy attachment options

      Usage: --copy-attachments-from file options --
   ...
   This flag starts copy attachment options, used to copy attachments
   from other files.

   Usage: :samp:`--copy-attachments-from {file} {options} --`
...
.. help-topic add-attachment: adding attachments
   ...
   either literally "Z" (upper case) for UTC or a timezone offset in
...
:samp:`{z}` is either literally ``Z`` (upper case) for UTC or a
...
.. qpdf:option:: --creationdate=date
   ...
   .. help: short text
      ...
      to the current time. Run qpdf --help=add-attachment for
      information about the date format.

   Specify the attachment's creation date in PDF format; defaults to
   the current time. See :ref:`add-attachment` for information about
   the date format.
...
.. qpdf:option:: --moddate=date
   ...
   .. help: short text
      ...
      defaults to the current time. Run qpdf --help=add-attachment
      for information about the date format.

   Specify the attachment's modification date in PDF format; defaults
   to the current time. See :ref:`add-attachment` for information
...
.. qpdf:option:: --mimetype=type/subtype
   ...
   Specify the mime type for the attachment, such as ``text/plain``,
   ``application/pdf``, ``image/png``, etc. Note that the mimetype
   appears in a field called ``/Subtype`` in the PDF but actually
   includes the full type and subtype of the mime type. This is
   because ``/Type`` already means something else in PDF.

   The qpdf library does not automatically determine the mime type. In
   a UNIX-like environment, the :command:`file` command can often
   provide this information. In MacOS, you can use :samp:`file -I
   {filename}`. In Linux, it's :samp:`file -i {filename}`.

.. qpdf:option:: --description="text"

   .. help: description for an attachment
...
.. help-topic copy-attachments: copying attachments
...
Option in this section are valid between
...
.. qpdf:option:: --is-encrypted
   ...
   .. help: silent test for encrypted file

      Silently exit with status 0 if the file is encrypted or status 2
      if the file is not encrypted. This can be used with
      password-protected files even if you don't know the password.

   Silently exit with status ``0`` if the file is encrypted or status
   ``2`` if the file is not encrypted. This is useful for shell
   scripts. Other options are ignored if this is given. This option is
   mutually exclusive with :qpdf:ref:`--requires-password`. Both this
   option and :qpdf:ref:`--requires-password` exit with status ``2`` for
   non-encrypted files. This option can be used for password-protected
   files even if you don't know the password.

.. qpdf:option:: --requires-password

   .. help: silent test for password
   ...
   requires no password or the correct password has been supplied.
   This is useful for shell scripts. The choice of exit status ``0``
   to mean that a password is required is to enable code like
   ...
   used with a :qpdf:ref:`--password` option, this option can be used to
   check the correctness of the password. In that case, an exit status
   of ``3`` means the file works with the supplied password. This
   option is mutually exclusive with :qpdf:ref:`--is-encrypted`. Both this
...
.. qpdf:option:: --check
   ...
   .. help: partially check whether PDF is valid
      ...
      aspects of the file, and write information to standard output
      about the file. Note that qpdf does not perform any validation
   ...
   encoding of stream data, writing information to standard output
   about the file. An exit status of ``0`` indicates syntactic
...
.. qpdf:option:: --show-encryption
   ...
   .. help: information about encrypted files
      ...
      Shows document encryption parameters. Also shows the document's
...
.. qpdf:option:: --show-linearization
   ...
   .. help: show linearization hint tables
      ...
      Check and displays all data in the linearization hint tables.

   Check and display all data in the linearization hint tables.

.. qpdf:option:: --show-xref

   .. help: show cross reference table

      Show the contents of the cross-reference table in a
      human-readable form. This is especially useful for files with
      cross-reference streams which are stored in a binary format.

   Show the contents of the cross-reference table in a human-readable
   form. This is especially useful for files with cross-reference
   streams which are stored in a binary format. If the file is
   invalid and cross reference table reconstruction is performed, this
   option will show the information in the reconstructed table.
...
.. qpdf:option:: --raw-stream-data
   ...
   .. help: show raw stream data
      ...
      instead of the object's contents.
...
.. qpdf:option:: --filtered-stream-data
   ...
   .. help: show filtered stream data
      ...
      standard output instead of the object's contents.
...
.. qpdf:option:: --json-key=key
   ...
   .. help: restrict json output

      This option is repeatable. If specified, only top-level keys
      specified will be included in the JSON output. If not specified,
      all keys will be shown.
...
.. qpdf:option:: --json-object=trailer|obj[,gen]
   ...
   .. help: restrict which objects are in JSON
      ...
      This option is repeatable. If specified, only specified objects
      will be shown in the "objects" key of the JSON output. If
      absent, all objects will be shown.
...
The options below are useful when writing automated test code that
includes files created by qpdf or when testing qpdf itself. Care is
taken to avoid gratuitously changing the output of PDF files to make
it easier to do direct comparisons in test suites with files created
by qpdf. However, there are no guarantees that the PDF output won't
change.
...
random element, and the interaction stream length encoding with
...
.. qpdf:option:: --static-id
   ...
   .. help: use a static document ID

      Causes generation of a fixed value for the document ID. This is
      intended for testing only. Never use it for production files.
      See also qpdf--help=--deterministic-id.

   Causes generation of a fixed value for the document ID (``/ID`` in
   the trailer). **This is intended for testing only. Never use it for
   production files.** If you are trying to get the same ID each time
   for a given file and you are not generating encrypted files,
   consider using the :qpdf:ref:`--deterministic-id` option.
...
.. qpdf:option:: --static-aes-iv
   ...
   Use of a static initialization vector for AES-CBC. This is intended
   for testing only so that output files can be reproducible. Never
...
Please note that the :qpdf:ref:`--password-is-hex-key`
